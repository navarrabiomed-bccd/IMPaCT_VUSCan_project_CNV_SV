...
and parsed TSV files for the SNV pipeline. For use with the IMPaCT families.
...
def build_sample_id_maps(family_id, file_path=SAMPLE_PHENO_FILE):
    ...
    with open(file_path, mode='r', encoding='utf-8', newline='') as file:
        ...
        for row in reader:
            impact_id = row.get('Muestra ID') or row.get('IMPACT_ID')
            cnag_id = row.get('ID_CNAG') or row.get('CNAG_ID')
            if not impact_id or not cnag_id:
                continue
            if pattern and pattern not in str(impact_id):
                continue
            imp_to_cnag[str(impact_id)] = str(cnag_id)
            cnag_to_imp[str(cnag_id)] = str(impact_id)

    if not imp_to_cnag:
        raise ValueError(f'No se encontraron muestras para la familia {family_id}')
        ...
        for row in reader:
            if row['Annotation_mode'] == 'full':
                ...
                if sv_id not in sv_dict:
                    sv_dict[sv_id] = {k: v for k, v in row.items() if k != 'AnnotSV_ID'}
...
def process_family_sv(folder_path, folder_results, node, family_id):
    ...
    try:
        for file_name in input_files:
            if node == 'CNAG' and file_name.endswith('.tsv'):
                # Obtener ID de la muestra
                cnag_id = file_name.split('.')[0]
            ...
            elif node == 'NASERTIC' and file_name.endswith('.tab'):
                file_path = os.path.join(folder_path, file_name)
                sample_id = file_name.split('.')[0]
                logging.info(f'Procesando SVs de muestra: {sample_id}')
...
def main(folder_path):
    ...
    if node == 'FPGMX':
        copy_somatic_folder(os.path.join(folder_path, 'CNV/INPUTS'), cnv_outputs, node, 'CNV')
    else:
        copy_somatic_folder(cnv_inputs, cnv_outputs, node, 'CNV')

    # Procesar CNVs de probando y familiares
    ...
    if not input_files:
        ...
        if node == 'CNAG':
            ...
            for file_name in input_files:
                if file_name.endswith('fixed.tsv'):
    ...
    # Copiar carpeta 'SOMATIC' con los archivos de somático
    if node == 'FPGMX':
        copy_somatic_folder(os.path.join(folder_path, 'SV/INPUTS'), sv_outputs, node, 'SV')
    else:
